// src/app/api/asset/import-used/route.ts
// API สำหรับ Import Asset USED (ของเก่า) พร้อมสร้าง AssetTransactionHistory
...
export async function POST(request: NextRequest) {
    try {
        ...
        let importDocument = await prisma.document.findFirst({
            where: { documentType: "IMPORT_USED" },
        });
        ...
        if (!importDocument) {
            ...
            importDocument = await prisma.document.create({
                data: {
                    docCode: `IMPORT_USED-${Date.now()}`,
                    documentType: "IMPORT_USED",
                    fullName: "System Import USED",
                ...
                try {
                    await prisma.securitySetTransaction.create({
                        data: {
                            ...
                            remarkIn: remarkIn || "Import Security Set USED",
                        },
                    });
            ...
            try {
                ...
                await prisma.assetTransactionHistory.create({
                    data: {
                        ...
                        mcsCodeIn: mcsCodeIn || null,
                        fromShop: fromShop || null,
                        remarkIn: remarkIn || "Import Asset USED",

                        // Auto fields
                        assetStatus: "USED", // ✅ สถานะเป็น USED
        ...
        // 8. สร้าง Response
        let message = `นำเข้า Asset USED สำเร็จ ${imported} รายการ`;
        if (skipped > 0) {
