// app/api/database-security-set/update/route.ts
// API สำหรับ Admin แก้ไขข้อมูลใน SecuritySetTransaction
  ...
  try {
    ...
    const results = await prisma.$transaction(
      validUpdates.map((item) =>
        prisma.securitySetTransaction.update({
          where: { id: item.id },
          data: item.data,
        })
      )
    ...
    console.error("Error updating security set database:", error);
    return NextResponse.json(
      {
        success: false,
