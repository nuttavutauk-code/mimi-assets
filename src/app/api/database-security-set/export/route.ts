// app/api/database-security-set/export/route.ts
// API สำหรับ Export ข้อมูล SecuritySetTransaction เป็น Excel
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 3. ดึงข้อมูลทั้งหมด
    const transactions = await prisma.securitySetTransaction.findMany({
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Security Set Database");

    // 6. สร้าง buffer
    const buffer = XLSX.write(workbook, { type: "buffer", bookType: "xlsx" });

    // 7. ส่งไฟล์กลับ
    const filename = `SecuritySet_Database_${new Date().toISOString().split("T")[0]}.xlsx`;
    ...
    console.error("Error exporting security set database:", error);
    return NextResponse.json(
