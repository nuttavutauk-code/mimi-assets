...
const FormShopToShop = ({ mode = "user" }: { mode?: FormMode }) => {
  ...
  const handleDraftSave = async () => {
    ...
    try {
      const filledSecuritySets = securitySets.filter(s => s.qty > 0);
      const payload = { documentType: "shop-to-shop", docCode: formData.docNumber, fullName: formData.fullName, company: formData.company, phone: formData.phone, note, status: "reviewing", shops: [{ shopCode: shopCodeFrom, shopName: shopNameFrom, startInstallDate: transferDate, assets: assets.map(a => ({ barcode: a.barcode, name: a.name, size: a.size, grade: a.grade, qty: a.qty })), securitySets: filledSecuritySets.map(s => ({ name: s.name, qty: s.qty })) }, { shopCode: shopCodeTo, shopName: shopNameTo }] };
