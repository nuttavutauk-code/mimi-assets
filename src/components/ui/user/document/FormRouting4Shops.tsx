...
type AssetRow = { id: number; name: string; size: string; kv: string; qty: number; withdrawFor: string; autoWarehouse?: boolean; useCustomSize?: boolean; customW?: string; customD?: string; customH?: string; customXX?: string; };
type SecuritySet = { id: number; name: string; qty: number; withdrawFor: string };
type ShopState = { noMcs: boolean; shopCode: string; shopName: string; startDate: string; endDate: string; q7b7: string; focus: string; searchResults: ShopItem[]; showDropdown: boolean; };
type FormMode = "user" | "admin";

const isCustomSizeAsset = (name: string) => { const lowerName = name.toLowerCase().replace(/\s+/g, ''); return lowerName.includes("lightbox") || lowerName.includes("accwall"); };
const defaultSecuritySets = (): SecuritySet[] => [{ id: 1, name: "CONTROLBOX 6 PORT (M-60000R) with power cable", qty: 0, withdrawFor: "" }, { id: 2, name: "CONTROLBOX 5 PORT (M-5000LD) with power cable", qty: 0, withdrawFor: "" }, { id: 3, name: "Security Type C Ver.7.1", qty: 0, withdrawFor: "" }, { id: 4, name: "Security Type C Ver.7.0", qty: 0, withdrawFor: "" }];
...
const FormRouting4Shops = ({ mode = "user" }: { mode?: FormMode }) => {
  ...
  const [dataLoaded, setDataLoaded] = useState(false); const [docStatus, setDocStatus] = useState(""); const [loading, setLoading] = useState(false); const [isSubmitting, setIsSubmitting] = useState(false);
  const assetIdCounter1 = useRef(1); const assetIdCounter2 = useRef(1); const assetIdCounter3 = useRef(1); const assetIdCounter4 = useRef(1);
  ...
  const [sizeOptions1, setSizeOptions1] = useState<Record<number, string[]>>({}); const [assetSearchResults1, setAssetSearchResults1] = useState<string[]>([]); const [showAssetDropdown1, setShowAssetDropdown1] = useState<Record<number, boolean>>({});
  ...
  const [sizeOptions2, setSizeOptions2] = useState<Record<number, string[]>>({}); const [assetSearchResults2, setAssetSearchResults2] = useState<string[]>([]); const [showAssetDropdown2, setShowAssetDropdown2] = useState<Record<number, boolean>>({});
  ...
  const [sizeOptions3, setSizeOptions3] = useState<Record<number, string[]>>({}); const [assetSearchResults3, setAssetSearchResults3] = useState<string[]>([]); const [showAssetDropdown3, setShowAssetDropdown3] = useState<Record<number, boolean>>({});

  const [shop4, setShop4] = useState<ShopState>({ noMcs: false, shopCode: "", shopName: "", startDate: "", endDate: "", q7b7: "", focus: "", searchResults: [], showDropdown: false });
  const [assets4, setAssets4] = useState<AssetRow[]>([{ id: assetIdCounter4.current++, name: "", size: "", kv: "", qty: 1, withdrawFor: "" }]);
  const [securitySets4, setSecuritySets4] = useState<SecuritySet[]>(defaultSecuritySets());
  const [sizeOptions4, setSizeOptions4] = useState<Record<number, string[]>>({}); const [assetSearchResults4, setAssetSearchResults4] = useState<string[]>([]); const [showAssetDropdown4, setShowAssetDropdown4] = useState<Record<number, boolean>>({});

  const abortRef = useRef<AbortController | null>(null);

  useEffect(() => { fetch("/api/vendor/list").then(r => r.json()).then(j => { if (j.success) setVendors((j.vendors || []).filter((v: string) => v && v.trim() !== "" && v !== "-")); }); }, []);
  ...
  const debouncedShopSearch4 = useMemo(() => debounce((q: string) => fetchShops(q, 4), 300), [fetchShops]);

  const fetchAssets = async (query: string, shopNum: 1 | 2 | 3 | 4, rowId: number) => { const setResults = shopNum === 1 ? setAssetSearchResults1 : shopNum === 2 ? setAssetSearchResults2 : shopNum === 3 ? setAssetSearchResults3 : setAssetSearchResults4; const setDropdown = shopNum === 1 ? setShowAssetDropdown1 : shopNum === 2 ? setShowAssetDropdown2 : shopNum === 3 ? setShowAssetDropdown3 : setShowAssetDropdown4; if (!query || query.length < 2) { setResults([]); setDropdown(p => ({ ...p, [rowId]: false })); return; } try { const res = await fetch(`/api/asset/search?query=${encodeURIComponent(query)}`); const json = await res.json(); const names = json.assets?.map((a: any) => a.assetName).filter(Boolean) ?? []; setResults(names); setDropdown(p => ({ ...p, [rowId]: true })); } catch (err) { console.error(err); } };
  ...
  const debouncedAssetSearch4 = useMemo(() => debounce((q: string, id: number) => fetchAssets(q, 4, id), 300), []);
  ...
  const handleSubmit = async (action: string) => {
    ...
    try {
      ...
      if (filledSec1.length > 3) { toast.error("Security Set Shop 1 กรอกได้สูงสุด 3 รายการเท่านั้น"); setIsSubmitting(false); return; }
      if (filledSec2.length > 3) { toast.error("Security Set Shop 2 กรอกได้สูงสุด 3 รายการเท่านั้น"); setIsSubmitting(false); return; }
      if (filledSec3.length > 3) { toast.error("Security Set Shop 3 กรอกได้สูงสุด 3 รายการเท่านั้น"); setIsSubmitting(false); return; }
      if (filledSec4.length > 3) { toast.error("Security Set Shop 4 กรอกได้สูงสุด 3 รายการเท่านั้น"); setIsSubmitting(false); return; }
  ...
  const renderShopCard = (shopNum: 1 | 2 | 3 | 4) => {
    const shop = shopNum === 1 ? shop1 : shopNum === 2 ? shop2 : shopNum === 3 ? shop3 : shop4;
    const setShop = shopNum === 1 ? setShop1 : shopNum === 2 ? setShop2 : shopNum === 3 ? setShop3 : setShop4;
    const debouncedSearch = shopNum === 1 ? debouncedShopSearch1 : shopNum === 2 ? debouncedShopSearch2 : shopNum === 3 ? debouncedShopSearch3 : debouncedShopSearch4;
    const colors = ['green', 'orange', 'blue', 'purple'];
    return (<div className="glass-card p-4 sm:p-5"><div className="flex items-center gap-2 mb-4"><div className={`icon-container ${colors[shopNum - 1]} !w-8 !h-8`}><Store className="w-4 h-4" /></div><h2 className="font-semibold">ข้อมูล Shop ที่ {shopNum}</h2></div><div className="space-y-4"><div className="flex flex-col sm:flex-row sm:items-end gap-4"><label className="flex items-center gap-2 cursor-pointer"><Checkbox checked={shop.noMcs} onCheckedChange={(c) => setShop(p => ({ ...p, noMcs: !!c, shopCode: "", shopName: "" }))} /><span className="text-sm font-medium">NO MCS</span></label><div className="flex-1 relative"><label className="block text-xs text-muted-foreground mb-1">MCS Code</label><Input value={shop.shopCode} onChange={(e) => { setShop(p => ({ ...p, shopCode: e.target.value })); debouncedSearch(e.target.value); }} onFocus={() => shop.searchResults.length > 0 && setShop(p => ({ ...p, showDropdown: true }))} onBlur={() => setTimeout(() => setShop(p => ({ ...p, showDropdown: false })), 200)} disabled={shop.noMcs} placeholder="พิมพ์ MCS Code..." className="glass-input" />{shop.showDropdown && shop.searchResults.length > 0 && (<div className="absolute top-full left-0 z-20 mt-1 w-full bg-white border rounded-xl shadow-lg max-h-48 overflow-auto">{shop.searchResults.map(s => (<div key={s.mcsCode} className="px-3 py-2 hover:bg-black/5 cursor-pointer text-sm" onClick={() => setShop(p => ({ ...p, shopCode: s.mcsCode, shopName: s.shopName, showDropdown: false }))}><span className="text-primary font-medium">{s.mcsCode}</span> - {s.shopName}</div>))}</div>)}</div><div className="flex-[2]"><label className="block text-xs text-muted-foreground mb-1">Shop Name <span className="text-red-500">*</span></label><Input value={shop.shopName} onChange={(e) => setShop(p => ({ ...p, shopName: e.target.value }))} disabled={!shop.noMcs} placeholder="ชื่อ Shop" className="glass-input" /></div></div><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><div><label className="block text-xs text-muted-foreground mb-1">วันที่เริ่มติดตั้ง <span className="text-red-500">*</span></label><Input type="date" value={shop.startDate} onChange={(e) => setShop(p => ({ ...p, startDate: e.target.value }))} className="glass-input" /></div><div><label className="block text-xs text-muted-foreground mb-1">วันที่ติดตั้งเสร็จ <span className="text-red-500">*</span></label><Input type="date" value={shop.endDate} onChange={(e) => setShop(p => ({ ...p, endDate: e.target.value }))} className="glass-input" /></div><div><label className="block text-xs text-muted-foreground mb-1">Q7B7 <span className="text-red-500">*</span></label><Select value={shop.q7b7} onValueChange={(v) => setShop(p => ({ ...p, q7b7: v }))}><SelectTrigger className="glass-input"><SelectValue placeholder="Yes / No" /></SelectTrigger><SelectContent><SelectItem value="Yes">Yes</SelectItem><SelectItem value="No">No</SelectItem></SelectContent></Select></div><div><label className="block text-xs text-muted-foreground mb-1">Shop Focus <span className="text-red-500">*</span></label><Select value={shop.focus} onValueChange={(v) => setShop(p => ({ ...p, focus: v }))}><SelectTrigger className="glass-input"><SelectValue placeholder="เลือกกลุ่ม" /></SelectTrigger><SelectContent><SelectItem value="1. Flagship">1. Flagship</SelectItem><SelectItem value="2. A Series (High to Flagship) 10K+">2. A Series (High to Flagship) 10K+</SelectItem><SelectItem value="3. A Series (Mid to High) 7-10K">3. A Series (Mid to High) 7-10K</SelectItem><SelectItem value="4. A Series (Mass) ~7K">4. A Series (Mass) ~7K</SelectItem><SelectItem value="5. A Series (Entry) < 5K">5. A Series (Entry) &lt; 5K</SelectItem></SelectContent></Select></div></div></div></div>);
  };

  const renderAssetCard = (shopNum: 1 | 2 | 3 | 4) => {
    const assets = shopNum === 1 ? assets1 : shopNum === 2 ? assets2 : shopNum === 3 ? assets3 : assets4;
    const setAssets = shopNum === 1 ? setAssets1 : shopNum === 2 ? setAssets2 : shopNum === 3 ? setAssets3 : setAssets4;
    const sizeOptions = shopNum === 1 ? sizeOptions1 : shopNum === 2 ? sizeOptions2 : shopNum === 3 ? sizeOptions3 : sizeOptions4;
    const assetSearchResults = shopNum === 1 ? assetSearchResults1 : shopNum === 2 ? assetSearchResults2 : shopNum === 3 ? assetSearchResults3 : assetSearchResults4;
    const showAssetDropdown = shopNum === 1 ? showAssetDropdown1 : shopNum === 2 ? showAssetDropdown2 : shopNum === 3 ? showAssetDropdown3 : showAssetDropdown4;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center gap-2"><div className="icon-container blue !w-10 !h-10"><Route className="w-5 h-5" /></div><div><h1 className="text-xl sm:text-2xl font-semibold">{isEdit ? "แก้ไข" : "ใบเบิก"} Routing 4 Shops</h1><p className="text-sm text-muted-foreground">เลขที่ {formData.docNumber || "รอสร้าง"}</p></div></div>
      ...
      {renderShopCard(1)}{renderAssetCard(1)}{renderSecuritySetCard(1)}
      {renderShopCard(2)}{renderAssetCard(2)}{renderSecuritySetCard(2)}
      {renderShopCard(3)}{renderAssetCard(3)}{renderSecuritySetCard(3)}
      {renderShopCard(4)}{renderAssetCard(4)}{renderSecuritySetCard(4)}
...
export default FormRouting4Shops;