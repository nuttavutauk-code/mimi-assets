...
import { Plus, Trash2, User, Store, Package, FileText, Save, CheckCircle, XCircle, Loader2, MoreHorizontal, Shield, X } from "lucide-react";
...
const FormOther = ({ mode = "user" }: { mode?: FormMode }) => {
      ...
      if (json.success) {
        ...
        if (shop) {
          ...
          if (shop.securitySets?.length > 0) {
            setSecuritySets(defaultSecuritySets.map(def => { const found = shop.securitySets.find((s: any) => s.name === def.name); return found ? { ...def, qty: found.qty || 0, withdrawFor: found.withdrawFor || "" } : def; }));
            if (doc.status === "reviewing") {
  ...
  const handleOpenPreview = () => {
    ...
    if (mode === "admin") {
      ...
      if (assetMissing) { toast.error(`กรุณาเลือก Barcode ให้ครบทุกรายการ (Asset: ${assetMissing.name})`); return; }
      ...
      if (secMissing) { toast.error(`กรุณาเลือก Barcode ให้ครบทุกรายการ (Security: ${secMissing.name})`); return; }
    }
  ...
  const handleSubmit = async (action: string) => {
    ...
    try {
      ...
      if (action === "approve" && mode === "admin") {
        ...
        if (assetMissing) { toast.error(`กรุณาเลือก Barcode ให้ครบทุกรายการ (Asset: ${assetMissing.name})`); setIsSubmitting(false); return; }
        const secMissing = filledSecuritySets
          .filter(s => !s.name.includes("Security Type C"))
          .find(s => {
            const arr = securityBarcodes[s.id] || [];
        ...
        if (secMissing) { toast.error(`กรุณาเลือก Barcode ให้ครบทุกรายการ (Security: ${secMissing.name})`); setIsSubmitting(false); return; }
      }
      ...
      if (action === "approve" && editId) {
        ...
        const filledAssetsForApprove = assets.filter(a => a.name && a.name.trim() !== "");
        const assignedBarcodes = {
          assetBarcodes: filledAssetsForApprove.map((a, idx) => ({ shopIndex: 0, assetIndex: idx, barcodes: (assetBarcodes[a.id] || []).slice(0, a.qty) })),
          securityBarcodes: filledSecuritySets.map((s, idx) => ({ shopIndex: 0, securityIndex: idx, barcodes: s.name.includes("Security Type C") ? [] : (securityBarcodes[s.id] || []).slice(0, s.qty) })),
        };
        const res = await fetch("/api/document/approve", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ documentId: parseInt(editId), otherActivity, assignedBarcodes }) }); const r = await res.json(); if (!r.success) throw new Error(r.message); toast.success("อนุมัติสำเร็จ!"); router.push("/dashboard/admin-list"); return;
      }
  ...
  const handleDraftSave = async () => {
    ...
    try {
      const filledAssets = assets.filter(a => a.name && a.name.trim() !== "");
      const payload = {
        documentType: "other", docCode: formData.docNumber, fullName: formData.fullName, company: formData.company, phone: formData.phone, note, status: "reviewing", transactionStatus: transactionStatus || null,
        shops: [{ shopCode, shopName, startInstallDate: startDate, endInstallDate: endDate, q7b7, shopFocus, assets: filledAssets.map(a => ({ name: a.name, size: a.size, grade: a.grade, qty: a.qty, withdrawFor: a.withdrawFor, barcode: JSON.stringify(assetBarcodes[a.id] || []) })), securitySets: securitySets.filter(s => s.qty > 0).map(s => ({ name: s.name, qty: s.qty, withdrawFor: s.withdrawFor, barcode: JSON.stringify(s.name.includes("Security Type C") ? [] : (securityBarcodes[s.id] || [])) })) }],
      };
    ...
    <div className="space-y-4 sm:space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                <div className="sm:col-span-4 relative">
                  ...
                  <div className="relative">
                    <Input value={asset.name} onChange={(e) => { if (!asset.isSelected) { setAssets(p => p.map(a => a.id === asset.id ? { ...a, name: e.target.value } : a)); debouncedAsset(e.target.value, asset.id); } }} onFocus={() => !asset.isSelected && assetSearchResults.length > 0 && setShowAssetDropdown(p => ({ ...p, [asset.id]: true }))} onBlur={() => setTimeout(() => setShowAssetDropdown(p => ({ ...p, [asset.id]: false })), 200)} placeholder="พิมพ์ชื่อ Asset..." readOnly={asset.isSelected} className={`glass-input ${asset.isSelected ? "pr-10 bg-gray-50" : ""}`} />
                    {asset.isSelected && (
                      <button type="button" onClick={() => setAssets(p => p.map(a => a.id === asset.id ? { ...a, name: "", size: "", isSelected: false } : a))} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full hover:bg-gray-200 text-gray-400 hover:text-gray-600" title="ล้างเพื่อเลือกใหม่"><X className="w-4 h-4" /></button>
                    )}
                  </div>
                  {showAssetDropdown[asset.id] && assetSearchResults.length > 0 && !asset.isSelected && <div className="absolute top-full left-0 z-20 mt-1 w-full bg-white border rounded-xl shadow-lg max-h-48 overflow-auto">{assetSearchResults.map(n => <div key={n} className="px-3 py-2 hover:bg-black/5 cursor-pointer text-sm" onMouseDown={(e) => { e.preventDefault(); setAssets(p => p.map(a => a.id === asset.id ? { ...a, name: n, isSelected: true } : a)); fetchSizes(n, asset.id); setShowAssetDropdown(p => ({ ...p, [asset.id]: false })); }}>{n}</div>)}</div>}
                </div>
      ...
      {!isReadOnly && (
        <div className="flex flex-col sm:flex-row justify-center gap-3 pt-4">
          {mode === "admin" ? (
            <><button disabled={isSubmitting} onClick={handleDraftSave} className="px-8 py-3 rounded-xl bg-purple-500/10 text-purple-600 border border-purple-200 text-sm font-medium flex items-center justify-center gap-2 hover:bg-purple-500/20 transition-colors disabled:opacity-50"><Save className="w-4 h-4" />{isSubmitting ? "กำลังบันทึก..." : "บันทึก Draft"}</button><button disabled={isSubmitting} onClick={handleOpenPreview} className="gradient-button px-8 py-3 text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50"><CheckCircle className="w-4 h-4" />{isSubmitting ? "กำลังดำเนินการ..." : "อนุมัติ"}</button><button disabled={isSubmitting} onClick={() => handleSubmit("reject")} className="px-8 py-3 rounded-xl bg-red-500 text-white text-sm font-medium flex items-center justify-center gap-2 hover:bg-red-600 disabled:opacity-50"><XCircle className="w-4 h-4" />ปฏิเสธ</button></>
          ) : (
