...
import { Library, Search, ChevronLeft, ChevronRight, Loader2, ImageIcon } from "lucide-react";
import { toast } from "sonner";
import { Dialog, DialogContent, DialogTitle } from "@/components/ui/dialog";
import { VisuallyHidden } from "@radix-ui/react-visually-hidden";

export default function UserLibrarySES() {
  ...
  useEffect(() => {
    fetchLibrarySES(search, page);
  }, [page]);

  const handleSearch = () => {
    setPage(1);
    fetchLibrarySES(search, 1);
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl sm:text-2xl font-semibold text-foreground">Library SES</h1>
        </div>
        <p className="text-sm text-muted-foreground">คลังข้อมูล Security Equipment Standard</p>
      </div>
      ...
      <div className="glass-card p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="ค้นหา Asset Name, Code, Barcode..."
              value={search}
              ...
              <table className="glass-table w-full">
                ...
                <tbody>
                  {data.map((item, idx) => (
                    <tr key={idx}>
                      <td>
                        <span className="px-2 py-1 rounded-md bg-purple-50 text-purple-600 text-xs font-medium">
                          {item.category}
                        </span>
                      </td>
                      <td>
                        {item.imageUrl ? (
                          <div
                            className="w-12 h-12 overflow-hidden rounded-lg cursor-pointer border border-black/5 hover:opacity-80 transition-opacity"
                            onClick={() => setSelectedImage(item.imageUrl)}
                          >
                            <Image
                              src={item.imageUrl}
                              alt={item.assetName || "image"}
                              width={48}
                              height={48}
                      ...
                      <td>
                        <span className={`status-badge ${item.status === 'Active' ? 'approved' : 'pending'}`}>
                          {item.status}
                        </span>
                      </td>
            ...
            {data.map((item, idx) => (
              <div key={idx} className="glass-card p-4">
                <div className="flex gap-3">
                  {item.imageUrl ? (
                    <div
                      className="w-16 h-16 flex-shrink-0 overflow-hidden rounded-lg cursor-pointer border border-black/5"
                      onClick={() => setSelectedImage(item.imageUrl)}
                    >
                      <Image
                        src={item.imageUrl}
                    ...
                    <div className="w-16 h-16 flex-shrink-0 bg-black/5 rounded-lg flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className={`status-badge !py-0.5 ${item.status === 'Active' ? 'approved' : 'pending'}`}>
                        {item.status}
                      </span>
                    </div>
                  </div>
