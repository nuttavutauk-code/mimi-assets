...
import { BookOpenText, Search, Loader2, ImageIcon, Warehouse, Calendar } from "lucide-react";
import { toast } from "sonner";
import { Dialog, DialogContent, DialogTitle } from "@/components/ui/dialog";
import { VisuallyHidden } from "@radix-ui/react-visually-hidden";

export default function UserLibrarySIS() {
  ...
  useEffect(() => {
    fetchLibrarySIS(search, page);
  }, [page]);

  const handleSearch = () => {
    setPage(1);
    fetchLibrarySIS(search, 1);
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="glass-card p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="ค้นหา Barcode, Asset Name, Warehouse..."
              value={search}
                      ...
                      <td>
                        {item.pictureUrl ? (
                          <div
                            ...
                            onClick={() => setSelectedImage(item.pictureUrl)}
                          >
                            <Image
                              src={item.pictureUrl}
      ...
      <div className="sm:hidden space-y-3">
        {loading ? (
          ...
          <div className="glass-card p-8 text-center text-muted-foreground">
            <BookOpenText className="w-10 h-10 mx-auto mb-2 opacity-30" />
            ไม่พบข้อมูล
          </div>
        ) : (
          <>
