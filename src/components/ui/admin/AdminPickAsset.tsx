...
export default function AdminPickAsset() {
  ...
  const fetchPickTasks = async (pageNum?: number) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/pick-asset/all-tasks?page=${page}&limit=10&search=${encodeURIComponent(search)}`
      );
  ...
  const handleViewTask = (documentId: number, shopCode: string) => {
    router.push(`/dashboard/admin-pick/${documentId}?shopCode=${encodeURIComponent(shopCode)}`);
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl sm:text-2xl font-semibold text-foreground">Pick Asset (Admin View)</h1>
        <p className="text-sm text-muted-foreground mt-0.5">ดูรายการงานหยิบ Asset ทั้งหมด (View Only)</p>
      </div>
      ...
      <div className="glass-card p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="ค้นหาเลขที่เอกสาร, โกดัง, ชื่อร้าน, ผู้รับผิดชอบ..."
              value={search}
                ...
                <div key={`${item.id}-${index}`} className="glass-card p-4">
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">{item.assignedUser || "-"}</span>
                    </div>
                    <div>
