...
import { Edit, Trash2, Download, Loader2, Search, ChevronLeft, ChevronRight, Filter, ShieldCheck } from "lucide-react";
...
export default function AdminList() {
  ...
  const getEditPath = (documentType: string, id: string) => {
    const pathMap: Record<string, string> = {
      ...
      routing2shops: `/dashboard/admin-document/routing2shops/${id}`,
      routing3shops: `/dashboard/admin-document/routing3shops/${id}`,
      routing4shops: `/dashboard/admin-document/routing4shops/${id}`,
      ...
      shoptoshop: `/dashboard/admin-document/shop-to-shop/${id}`,
      "shop-to-shop": `/dashboard/admin-document/shop-to-shop/${id}`,
      repair: `/dashboard/admin-document/repair/${id}`,
    };
    return pathMap[documentType] || `/dashboard/admin-document/edit/${id}`;
  };
  ...
  const handleDownload = async (item: any) => {
    ...
    try {
      ...
      if (success) toast.success("ดาวน์โหลดเอกสารสำเร็จ");
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl sm:text-2xl font-semibold text-foreground">Document List</h1>
          </div>
          <p className="text-sm text-muted-foreground">Admin Panel - จัดการเอกสารทั้งหมดในระบบ</p>
        </div>
                      ...
                      <tr key={i}>
                        ...
                        <td>
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-pink-400 to-orange-400 flex items-center justify-center text-white text-xs font-medium">
                              {item.createdByUsername?.substring(0, 1).toUpperCase() || "?"}
                            </div>
                            <span className="text-sm">{item.createdByUsername || "-"}</span>
                          </div>
                        </td>
                        <td>{item.company || "-"}</td>
                ...
                <div key={i} className="glass-card p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">โดย {item.createdByUsername || "-"}</p>
                    </div>
